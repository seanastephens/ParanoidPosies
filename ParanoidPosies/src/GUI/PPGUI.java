...
public class PPGUI extends JFrame implements Runnable {
	...
	public void run() {
		while (true) {
			...
			if (game.getHive().isDead()) {
				BufferedImage image = (BufferedImage) ImageReg.getInstance().getImageFromStr(
						"DeadBees");
				ImageBackgroundPane pane = new ImageBackgroundPane(image);
				pane.showMessageDialog(null,
						"You lost! Feel free to enjoy the music by not clicking :D");
				System.exit(0);
			}
			repaint();
		}
	...
	private class ImageBackgroundPane extends JOptionPane {
		...
		public void paint(Graphics g) {
			...
			// Be careful, if the image is smaller than the JOptionPane size
			// you
			// will see a solid white background where the image does not
			// reach.
			// g.drawImage(img, 0, 0, img.getWidth(), img.getHeight());

			// Option 2:
			// If the image can be guaranteed to be larger than the
			// JOptionPane's size
			Dimension curSize = this.getSize();
			g.drawImage(img, 0, 0, curSize.width, curSize.height, null);

			// Make sure to paint all the other properties of Swing
			// components.
			super.paint(g);
		}
	}
